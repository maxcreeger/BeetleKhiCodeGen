...
public class ProcessLinker {
	...
	private Map<String, Operation> buildOperationLookup(Khiprocess process) throws InvalidKhiProcessException {
		...
		for (Operation operation : process.getPlan()
											.getOperations()
											.getOperation()) {
	...
	public ProcessOverview assemble() throws MissingKhiModuleException, InvalidKhiProcessException, UnavailableCommandException {
		...
		Map<String, LinkedNode> linkedNodeLookup = linkNodesToModues(root);
		...
		for (Operation operation : process.getPlan()
			...
			for (ExecuteCommand exec : operation.getExecuteCommand()) {
				...
				for (Command availableCommand : linkedNode.getKhiModule()
															.getCommunication()
															.getCommands()
															.getCommand()) {
					if (exec.getName()
	...
	private void prepareErrorEvent(Map<String, LinkedNode> linkedNodeLookup, Operation operation, LinkedOperation linkedOperation, Error error)
		...
		Event event = null;
		if (module.getCommunication()
					.getEvents() == null || module.getCommunication()
													.getEvents()
													.getEvent() == null || module.getCommunication()
																					.getEvents()
																					.getEvent()
																					.isEmpty()) {
			throw new InvalidKhiProcessException("Process '" + process.getName() + "', operation '" + operation.getName() + "' defines error '"
		...
		for (Event eventOfModule : module.getCommunication()
											.getEvents()
											.getEvent()) {
	...
	private LinkedCommand findCommand(LinkedOperation linkedOperation, ExecuteCommand exec, LinkedNode linkedNode) throws InvalidKhiProcessException {
		...
		for (Command command : linkedNode.getKhiModule()
											.getCommunication()
											.getCommands()
											.getCommand()) {
			if (exec.getName()
	...
	private Map<String, LinkedNode> linkNodesToModues(ProcessOverview root) throws MissingKhiModuleException {
